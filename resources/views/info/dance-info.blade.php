...
<head>
    ...
    <title>Traditional Dance - Wonderful Indonesia</title>
    @vite(['resources/css/dance-info/app.css', 'resources/js/dance-info/app.js'])
</head>
<body class="bg-gray-50">
    ...
    <div x-data="danceInfo" x-init="init()" class="pt-20">
        ...
        <div class="relative h-[60vh]">
            ...
            <div class="absolute bottom-0 left-0 right-0 p-8">
                <div class="container mx-auto">
                    <h1 class="text-4xl font-bold text-white mb-2" x-text="dance?.name || 'Loading...'"></h1>
                    <p class="text-xl text-white/90" x-text="dance?.provinsi || 'Indonesia'"></p>
                </div>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4 py-12">
            <div class="max-w-4xl mx-auto space-y-8">
                ...
                <!-- Makna -->
                <div class="bg-white rounded-xl shadow-lg p-8">
                    <h2 class="text-2xl font-bold mb-6">Meaning & Philosophy</h2>
                    <template x-if="!meaning">
                        <div class="flex items-center space-x-2">
                            <div class="animate-spin rounded-full h-4 w-4 border-2 border-purple-500 border-t-transparent"></div>
                            <span class="text-gray-600">Analyzing cultural significance...</span>
                        </div>
                    </template>
                    <div class="prose max-w-none" x-show="meaning" x-html="meaning"></div>
                </div>

                <!-- Keunikan -->
                <div class="bg-white rounded-xl shadow-lg p-8">
                    <h2 class="text-2xl font-bold mb-6">Unique Features</h2>
                    <template x-if="!uniqueness">
                        <div class="flex items-center space-x-2">
                            <div class="animate-spin rounded-full h-4 w-4 border-2 border-purple-500 border-t-transparent"></div>
                            <span class="text-gray-600">Discovering unique features...</span>
                        </div>
                    </template>
                    <div class="prose max-w-none" x-show="uniqueness" x-html="uniqueness"></div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <template x-for="(image, index) in supportingImages" :key="index">
                            <div class="relative aspect-ratio-4-3 rounded-lg overflow-hidden">
                                <img 
                                    :src="image" 
                                    :alt="dance ? dance.name + ' image ' + (index + 1) : 'Dance image'"
                                    class="w-full h-full object-cover"
                                >
                            </div>
                        </template>
