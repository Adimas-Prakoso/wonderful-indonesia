...
<head>
    ...
    <title>Traditional Foods - Wonderful Indonesia</title>
    @vite(['resources/css/explore/app.css', 'resources/js/food/app.js'])
</head>
<body class="bg-gray-50">
    ...
    <!-- Main Content -->
    <div class="pt-16 pb-12" x-data="foods" x-init="init()">
        <!-- Hero Section -->
        <div class="relative bg-gradient-to-r from-green-600 to-emerald-500 h-[300px] mb-12">
            <div class="absolute inset-0 bg-black/30"></div>
            <div class="relative container mx-auto px-4 h-full flex flex-col justify-center">
                <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">Traditional Indonesian Foods</h1>
                <p class="text-xl text-white/90 max-w-2xl">Savor the authentic flavors of Indonesia's diverse culinary heritage.</p>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-8 mb-12 transition-all duration-300 hover:shadow-xl space-y-6">
                <!-- Search Input -->
                <div class="relative">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Search Traditional Foods</label>
                    <div class="relative">
                        <input 
                            type="text" 
                            x-model="searchQuery"
                            ...
                            class="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-300"
                        >
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select 
                            x-model="sortOrder"
                            @change="filterFoods"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-300"
                        >
            ...
            <!-- No Results State -->
            <div x-show="filteredFoods.length === 0 && !loading" class="text-center py-12">
                <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <h3 class="mt-4 text-lg font-medium text-gray-900">No traditional foods found</h3>
                <p class="mt-2 text-gray-500">Try adjusting your search or filter criteria</p>
            </div>

            <!-- Foods Grid -->
            <div x-show="filteredFoods.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                <template x-for="food in filteredFoods" :key="food['food-name']">
                    <a :href="food.link" class="group">
